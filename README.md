...
Clarity 是面向工业视觉 AI 模型交付环节的第三方技术验收平台，解决工业视觉模型 B2B 交付中的核心信任问题：**验收标准模糊、履约责任难界定、事后举证困难等痛点**。

系统采用**链上存证、链下计算**的混合架构，在不泄露原始数据与模型权重的前提下，对供应商交付的工业缺陷检测模型进行自动化审计，讲模型能力转化为不可篡改的可信凭证，并基于智能合约完成分阶段资金托管与履约结算。

## 核心特性

- **隐私保护视觉模型审计**  
  制造商测试集加密存储于 IPFS，仅 Hash 上链；审计在本地隔离环境中完成，计算结束后立即销毁环境，原始数据零泄露。

- **可解释性算法验证**  
  引入 D-RISE 可解释性算法，验证模型准确率的同时通过注意力热力图审查模型是否基于真实缺陷纹理做出判断，识别潜在的捷径学习与后门攻击。
